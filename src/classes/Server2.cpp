    ...
    if (command == "PASS") {
		...
		for (size_t i = 0; i < _channels.size(); i++) {
			...
			for (size_t j = 0; j < clients.size(); j++) {
				std::string user = clients[j]->getNickname();
				std::string listMsg = ":server 322 " + client->getNickname() + " " + channelName + " " + user + "\r\n";
				send(client->getFd(), listMsg.c_str(), listMsg.size(), 0);
			}
		}
		...
		if (channel) {
			...
			for (size_t i = 0; i < clients.size(); i++) {
				...
				std::string whoMsg = ":server 352 " + client->getNickname() + " " + target + " " + user->getUsername() + " " + user->getHostname() + " " + "localhost " + user->getNickname() + " H :0 " + user->getRealname() + "\r\n";
